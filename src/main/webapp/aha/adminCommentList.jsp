...
<display:table name="list" pagesize="4" class="its" requestURI="/servlet/CommentServlet" id="c">
	...
	<display:column title="操作">
		<a href="<%=request.getContextPath()%>/servlet/CommentServlet?method=edit&id=${c.id}"><img src="<%=request.getContextPath()%>/aha/images/edit.gif " border="0"/></a>
		<a href="<%=request.getContextPath()%>/servlet/CommentServlet?method=delete&id=${c.id}" onclick="javascript:return del()"><img src="<%=request.getContextPath()%>/aha/images/delete.gif" border="0"/></a>
	</display:column>
</display:table>
